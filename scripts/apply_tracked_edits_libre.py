        ...
        for row in rows_from_file(csv_path):
            ...
            if find == "<Company name, address>" and count_replaced > 0:
                try:
                    ...
                    replacement_length = len(repl)  # Length of "Consultex GmbH, Salvatorplatz 3, 80333 München"
                    extra_chars = replacement_length - original_length
                    
                    if extra_chars > 0:
                        print(f"🔧 Replacement is {extra_chars} characters longer than original - removing {extra_chars} trailing spaces to prevent logo overflow...")
                        
                        # Find the replacement text and remove exactly that many trailing spaces
                        search_desc = doc.createSearchDescriptor()
                        search_desc.SearchString = repl
                        search_desc.SearchCaseSensitive = False
                        search_desc.SearchWords = False
                        ...
                        while found_range:
                            try:
                                # Position cursor right after the replacement text
                                cursor = found_range.getText().createTextCursorByRange(found_range)
                                cursor.collapseToEnd()  # Move to end of the found replacement text
                                
                                # Count available whitespace characters following the replacement
                                available_spaces = 0
                                ...
                                for i in range(max_check):
                                    try:
                                        ...
                                        if not temp_cursor.goRight(1, True):
                                ...
                                if spaces_to_remove > 0:
                                    ...
                                    cursor.goRight(spaces_to_remove, True)
                                    cursor.setString("")  # Delete the selected whitespace
                                    total_spaces_removed += spaces_to_remove
                                    
                                    print(f"✅ Removed {spaces_to_remove} trailing whitespace characters after company name/address")
                                else:
                                    print(f"⚠️  No trailing whitespace available to remove after replacement")
                                    
                            except Exception as e:
                                print(f"⚠️  Could not remove spaces after replacement: {e}")
                            
                            found_range = doc.findNext(found_range, search_desc)
                        
                        if total_spaces_removed > 0:
                            print(f"🎉 Total of {total_spaces_removed} trailing characters removed to compensate for longer replacement text")
                    else:
                        print(f"✅ Replacement text is not longer than original - no space removal needed")
                        
                except Exception as e:
                    print(f"⚠️  Failed to apply space removal for company name/address: {e}")
