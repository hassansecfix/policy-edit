...
def main():
    ...
    try:
        ...
        for row in rows_from_file(csv_path):
            ...
            # Add comment if provided and replacements were made - attach directly to tracked change
            if comment_text and count_replaced > 0:
                try:
                    # Attach comment directly to the most recent tracked change (the replacement we just made)
                    redlines = doc.getPropertyValue("Redlines")
                    if redlines and redlines.getCount() > 0:
                        last_redline = redlines.getByIndex(redlines.getCount() - 1)
                        last_redline.setPropertyValue("Comment", f"{author_name}: {comment_text}")
                        print(f"✅ Added comment to tracked change by {author_name}")
                        print(f"   Comment: {comment_text[:100]}...")
                    else:
                        print(f"❌ No tracked changes available to attach comment to")
                        
                except Exception as e:
                    print(f"Warning: Could not add comment to tracked change: {e}")
